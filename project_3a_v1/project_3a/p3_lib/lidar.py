...
# Robotarium arena bounds and obstacle segments
ARENA: List[Segment] = [
    ((-1.6, -1.0), ( 1.6, -1.0)), # bottom
    (( 1.6, -1.0), ( 1.6,  1.0)), # right
    (( 1.6,  1.0), (-1.6,  1.0)), # top
    ((-1.6,  1.0), (-1.6, -1.0)), # left
]
OBSTACLES: List[Segment] = [
    ((-0.5, -0.5), (-0.5,  0.5)), # vertical wall at x=-0.5
    (( 0.3, -0.3), ( 0.9, -0.3)), # horizontal wall at y=-0.3
    (( 0.2,  0.2), ( 0.7,  0.6)), # diagonal wall
]
...
def simulate_lidar(
    ...
    for i, wangle in enumerate(world_angles):
        ...
        for seg in obstacles:
            ...
            # solve via 2D cross products
            d_x_ba = d[0] * ba[1] - d[1] * ba[0]
            if abs(d_x_ba) < 1e-9:
                continue  # parallel

            ap = a - p
            t = (ap[0] * ba[1] - ap[1] * ba[0]) / d_x_ba  # ray parameter
            s = (ap[0] * d[1] - ap[1] * d[0]) / d_x_ba    # segment parameter
